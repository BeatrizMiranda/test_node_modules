...
class HooksHandler extends _default2.default {
    ...
    hooks.forEach(hook => {
      ...
      if (current) {
        ...
        Object.keys(newSecrets).forEach(key => {
          ...
          } else if (newSecrets[key] !== _constants2.default.HOOKS_HIDDEN_SECRET_VALUE) {
        ...
        if (Object.keys(create).length) changes.create.push({
          hookId: current.id,
          secrets: create
        });
        if (Object.keys(update).length) changes.update.push({
          hookId: current.id,
          secrets: update
        });
        if (del.length) changes.del.push({
          hookId: current.id,
          secrets: del
        });
      }
  ...
  async getType(reload) {
    ...
    try {
      ...
      this.existing = await Promise.all(hooks.map(hook => this.client.hooks.get({
        id: hook.id
      }).then(hookWithCode => this.client.hooks.getSecrets({
        id: hook.id
      }).then(secrets => ({ ...hookWithCode,
        secrets
      })))));
      return this.existing;
    } catch (err) {
